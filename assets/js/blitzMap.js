...
var BlitzMap = new function(){
  ...
  this.init = function() {
      ...
      var mapOptions = {
          center: new google.maps.LatLng( 13.756951080090948, 100.49937991601655),
          zoom: 15,
  ...
  function setMapData( jsonString ){
    ...
    //   }
      ...
      for( var m = inputData.overlays.length-1; m >= 0; m-- ){
        ...
        mapOverlays.push(tmpOverlay); //save the overlay in the array

        var myPaths = tmpOverlay.getPaths();
          
        myPaths.forEach(function(path, i){

            // let drawingData = path.getArray();
            ...
            //     jQuery("textarea#mapData").val(updatedLatLong);

            //     console.log('currentData',currentData);
            //     console.log(index,'newData',newData);
            //     console.log('updatedLatLong',updatedLatLong);

            //   });

            google.maps.event.addListener(path, 'set_at', processVertex);

            google.maps.event.addListener(path, 'insert_at', processVertex);
            
            function processVertex(e) {
                jQuery("input#isInitial").val(2);
                let latlongArr = [];
                ...
                jQuery("textarea#mapData").val(verifiedLatlng);
            }
